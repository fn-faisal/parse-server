...
export class ExportRouter extends PromiseRouter {
  ...
  exportClass(req, data) {
    ...
    if (hasJoin) {
      findPromise = databaseController.adapter.count(
        data.name,
        relationSchema,
        data.where
      );
    } else {
      findPromise = rest.find(req.config, req.auth, data.name, data.where, {
        count: true,
        limit: 0,
      });
    }
  ...
  handleExportProgress(req) {
    ...
    return databaseController
      .find(DefaultExportExportProgressCollectionName, query)
      .then(response => {
        return { response };
      });
  }
  ...
  handleExport(req) {
    ...
    databaseController
      .create(DefaultExportExportProgressCollectionName, exportProgress)
      .then(() => {
        return databaseController.loadSchema({ clearCache: true });
      })
      .then(schemaController =>
        schemaController.getOneSchema(req.body.name, true)
      )
      .then(schema => {
      ...
      .then(jsonFiles => {
        return new Promise(resolve => {
          ...
          jsonFiles.forEach(tmpJsonFile => {
            zip.append(fs.readFileSync(tmpJsonFile.name), {
      ...
      .then(() => {
        return databaseController.destroy(
          DefaultExportExportProgressCollectionName,
          exportProgress
        );
      });
