...
function RestWrite(
  ...
  if (auth.isReadOnly) {
    throw new Parse.Error(
      ...
      ErrorMessage.READ_ONLY_MASTER_KEY()
    );
  }
  this.config = config;
  this.auth = auth;
  ...
  if (!query && data.objectId && !allowObjectId) {
    throw new Parse.Error(
      ...
      ErrorMessage.MISSING_OBJECT_ID()
    );
  }
...
RestWrite.prototype.validateAuthData = function () {
  ...
  throw new Parse.Error(
    ...
    ErrorMessage.UNSUPPORTED_SERVICE()
  );
};
    ...
    .then((results) => {
      ...
      if (this.query && this.query.objectId) {
        if (!objectIdMatch) {
          throw new Parse.Error(
            Parse.Error.OBJECT_NOT_FOUND,
            ErrorMessage.UNKNOWN_OBJECT_UPDATE()
          );
        }
        if (
          this.data.installationId &&
    ...
    .then(() => {
      if (!idMatch) {
        if (!deviceTokenMatches.length) {
          ...
          throw new Parse.Error(
            132,
            ErrorMessage.INSTALLATION_ID_WITH_DEVICE_TOKEN()
          );
        } else {
