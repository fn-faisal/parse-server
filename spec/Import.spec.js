...
describe('Import routers', () => {
  ...
  it('import objects with all data types', done => {
    const headers = {
      'Content-Type': 'multipart/form-data',
      'X-Parse-Application-Id': 'test',
      'X-Parse-Master-Key': 'test',
    ...
    request({
      ...
      .then(() => {
        ...
        query.find().then(results => {
          ...
          expect(results[0].get('dateColumn')).toEqual(
            new Date('2016-10-30T12:03:56.848Z')
          );
  ...
  it('send success import mail', done => {
    const emailAdapter = {
      sendMail: ({ text, to, subject }) => {
        expect(text).toEqual(
          'We have successfully imported your data to the class TestObject.'
        );
    ...
    reconfigureServer({
      ...
      request({
        ...
        .then(res => {
          expect(JSON.parse(res.body).response).toEqual(
  ...
  it('import relations object from file', done => {
    ...
    Parse.Object.saveAll([object, relatedObject])
      ...
      .then(savedObj => {
        ...
        request({
          headers: headers,
          url: 'import_relation_data/TestObjectDad/RelationObject',
          body: {
            importFile: {
              value: Buffer.from(
                JSON.stringify({
  ...
  it('send success import mail in the import relation', done => {
    const object = new Parse.Object('TestObjectDad');
    const relatedObject = new Parse.Object('TestObjectChild');
    const ids = {};
    Parse.Object.saveAll([object, relatedObject])
      ...
      .then(savedObj => {
        ...
        const emailAdapter = {
          sendMail: ({ text, to, subject }) => {
            expect(text).toEqual(
              'We have successfully imported your data to the class TestObjectDad, relation RelationObject.'
            );
            expect(to).toEqual('my@email.com');
            expect(subject).toEqual('Import completed');
            object
